...
| External-target mode (`bin/action-check.mjs run … --input`) against three Google WebMCP demos, Chrome 152 | Sports storefront `add_search_result_to_cart` retry: FAIL `DUPLICATE_EFFECT` (cart 0 → 2); Smart Home `rearrangeDOMComponents` with an unknown id: FAIL `FALSE_SUCCESS` (reply claims success, dashboard 1 → 0) and with two real ids: PASS `EFFECT_CONFIRMED` (1 → 2); zaMaker `add_topping` retry: FAIL `DUPLICATE_EFFECT` (0 → 2); `remove_topping` on an empty pizza: PASS `HONEST_REFUSAL`; `set_pizza_size` retry: PASS `IDEMPOTENT`. Proof JSON in `docs/evidence/external-targets-2026-09-03/`. 13 unit tests on the generic verdict |
...
| External-target mode, afternoon run against four more Google demos (Ticket booking, Luxe Leather, Analytics dashboard, Explainer), Chrome 152 | Ticket booking `select_showtime` with a real movie id: FAIL `FALSE_SUCCESS` (reply says proceed to checkout, checkout stays hidden); with an invalid id: PASS `HONEST_REFUSAL`; `update_location` retry: PASS `IDEMPOTENT`; Luxe Leather `add_to_cart` retry: FAIL `DUPLICATE_EFFECT` (cart page 0 → 2); Analytics `query` retry: PASS `IDEMPOTENT`; Explainer `cancelBooking` invalid id: PASS `HONEST_REFUSAL`. Six further runs set aside because the verdict could not be trusted (see `docs/audits/2026-09-03-ui-and-reality-audit.md`, section 8). Claim heuristic fixed for `{"status":"error"}` replies, 2 new unit tests |
| UI fixes from the 3 September audit (branch `feat/ui-audit-fixes`) | "Run it on any page" section with the twelve recorded runs, header nav, H1 fix, copy fixes; 167 unit + 2 CLI, 20/20 e2e with Axe, 1/1 native, public boundary clean |
